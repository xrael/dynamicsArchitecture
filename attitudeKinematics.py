...
def DCMrate2angVel(dcm, dcm_dot):
    ...
    :param dcm_dot: [2-dimension numpy array] Derivative of the DCM matrix.
    :return:
    """
    w_tilde = -dcm_dot.dot(dcm.T)
    return getVectorFromSkewSymmetricMatrix(w_tilde)

def DCMdoubleRate2angVelDot(dcm, dcm_ddot, w):
    """
    if w = w_BN_B (angular velocity of B frame relative to N expressed in B frame), this functions returns
    the derivative of w relative to B, which is equal to the derivative with respect to N.
    :param dcm:  [2-dimension numpy array] DCM matrix.
    :param dcm_ddot:  [2-dimension numpy array] Double derivative of the DCM matrix.
    :param w: Angular velocity. If DCM = [BN], w must be w_BN_B.
    :return:
    """
    w_tilde = getSkewSymmetrixMatrix(w)
    w_tilde_dot = -dcm_ddot.dot(dcm.T) + w_tilde.dot(w_tilde)
    return getVectorFromSkewSymmetricMatrix(w_tilde_dot)
...
def dcm2quat(dcm):
    """
    Computes the DCM from the quaternion using Sheppard's method (See Schaub-Junkins).
    :param dcm:
    :return:
    """
    beta_2 = np.zeros(4)
...
def computeErrorQuat(quat_BN, quat_RN):
    """
    ...
    G_T = np.array([[quat_RN[0], quat_RN[1], quat_RN[2], quat_RN[3]],
                    [-quat_RN[1], quat_RN[0], quat_RN[3], -quat_RN[2]],
                    [-quat_RN[2], -quat_RN[3], quat_RN[0], quat_RN[1]],
                    [-quat_RN[3], quat_RN[2], -quat_RN[1], quat_RN[0]],
                    ])

    quat_BR = G_T.dot(quat_BN)
    return quat_BR
...
def getBmatrix(mrp):
    """
    Computes the B matrix used in mrp_dot = 1/4 * B * w
    :param mrp:
    :return:
    """
    mrp_inner = np.inner(mrp, mrp)
...
def mrpRate2AngVel(mrp, mrp_dot):
    ...
    return (4.0/(1 + mrp_inner)**2 * B.T.dot(mrp_dot))


def switchMRPrepresentation(mrp):
    """
...
def computeErrorMRP(mrp_BN, mrp_RN):
    ...
    :param mrp_BN: Attitude of frame B relative to N.
    :param mrp_RN: Attitude of frame R relative to N.
    :return: Attitude of frame B relative to R.
    """
    ...
    mrp_BR = ((1-mrp_RN2) * mrp_BN - (1-mrp_BN2) * mrp_RN + 2 * np.cross(mrp_BN, mrp_RN))/(1 + mrp_RN2*mrp_BN2 + 2*np.dot(mrp_RN, mrp_BN))
