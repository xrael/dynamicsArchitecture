...
y_1 = np.array([0, 1, 0])       # 1st Instrument boresight in body frame
y_2 = np.array([1, 0, 0])       # 2nd Instrument boresight in body frame
...
control.addExclusionConstraint(x_2, y_1, cone_angle_2, 'Constraint_2')#, angle_thres_2, angle_thres_2_max)
control.addExclusionConstraint(x_3, y_1, cone_angle_3, 'Constraint_3')#, angle_thres_3, angle_thres_3_max)
# control.addExclusionConstraint(x_4, y_1, cone_angle_4, 'Constraint_4')#, angle_thres_4, angle_thres_4_max)

# control.addExclusionConstraint(x_1, y_2, cone_angle_1, 'Constraint_5')#, angle_thres_1, angle_thres_1_max)
# control.addExclusionConstraint(x_2, y_2, cone_angle_2, 'Constraint_6')#, angle_thres_2, angle_thres_2_max)
# control.addExclusionConstraint(x_3, y_2, cone_angle_3, 'Constraint_7')#, angle_thres_3, angle_thres_3_max)
# control.addExclusionConstraint(x_4, y_2, cone_angle_4, 'Constraint_8')#, angle_thres_4, angle_thres_4_max)
...
# mrp_BN_0 = np.array([-0.3,0, np.tan(np.deg2rad(120)/4)])
# w_BN_B_0 = np.array([0,0,0])
...
# y_N_2 = control_force_history['Constraint_5_y_N']
...
l = np.size(y_N_1,0)
dec_1 = np.zeros(l-1)
right_as_1 = np.zeros(l-1)
# dec_2 = np.zeros(l-1)
# right_as_2 = np.zeros(l-1)
...
for i in range(0, l-1):
    y_N_1_norm = np.linalg.norm(y_N_1[i])
    # y_N_2_norm = np.linalg.norm(y_N_2[i])
    # y_N_inc_norm = np.linalg.norm(y_N_inc[i])

    dec_1[i] = np.arcsin(y_N_1[i,2]/y_N_1_norm)
    right_as_1[i] = np.arctan2(y_N_1[i,1], y_N_1[i,0])

    # dec_2[i] = np.arcsin(y_N_2[i,2]/y_N_2_norm)
    # right_as_2[i] = np.arctan2(y_N_2[i,1], y_N_2[i,0])
...
plt.figure()
plt.hold(True)
plt.plot(time[::10], np.rad2deg(w_BR_B_dot_desired[::10,0]), label=r'$\dot\omega_{1d}$', color='r')
plt.plot(time[::10], np.rad2deg(w_BR_B_dot_desired[::10,1]), label=r'$\dot\omega_{2d}$', color='g')
plt.plot(time[::10], np.rad2deg(w_BR_B_dot_desired[::10,2]), label=r'$\dot\omega_{3d}$', color='b')
plt.xlabel(r'$t$ [sec]', size=27)
plt.ylabel(r'$\dot\omega_{B/R}$ [$^\circ$/sec]', size=27)
plt.legend(prop={'size':22})
...
plt.figure()
plt.hold(True)
plt.plot(time[::10], V_R[::10,0],label=r'$v_{R1}$')
plt.plot(time[::10], V_R[::10,1],label=r'$v_{R2}$')
plt.plot(time[::10], V_R[::10,2],label=r'$v_{R3}$')
plt.xlabel(r'Time [sec]')
plt.ylabel(r'$v_R$', size=18)
plt.legend()
...
# plt.scatter([5.8], [0], s=80, facecolors='none', edgecolors='k')
# ax.annotate(r'Saddle-Point', size = 25, xy=(5.8, 0), xytext=(60, 1),
#             arrowprops=dict(facecolor='black', shrink=0.02, width=1))
plt.xlim([t0, tf])
plt.ylim([0,2])
plt.xlabel(r'$t$ [sec]', size=27)
plt.ylabel(r'$||v_R||$', size=27)
plt.legend()
...
plt.figure()
plt.hold(True)
plt.plot(time, Uvect[:,0],label=r'$u_1$')
plt.plot(time, Uvect[:,1],label=r'$u_2$')
plt.plot(time, Uvect[:,2],label=r'$u_3$')
plt.xlabel("Time [sec]")
...
# ax.add_patch(patch4)
plt.hold(True)
plt.plot(right_as_1[::10]*180/np.pi, dec_1[::10]*180/np.pi,'.',markersize=2,label='Camera 1 vector tip')
plt.plot([right_as_1[0]*180/np.pi], [dec_1[0]*180/np.pi], marker='x', markersize=10, color="r",label='start')
plt.plot([right_as_1[-1]*180/np.pi], [dec_1[-1]*180/np.pi], marker='x', markersize=10, color="g",label='end')
ax.text(-97, -76, r'$1$',size=35)
ax.text(-97, -5, r'$2$',size=35)
ax.text(38, -5, r'$3$',size=35)
# ax.text(128, -5, r'$4$',size=35)
plt.plot([right_as_1[0]*180/np.pi, right_as_1[0]*180/np.pi], [dec_1[0]*180/np.pi, -90], '--k')
plt.plot([right_as_1[-1]*180/np.pi, right_as_1[-1]*180/np.pi], [dec_1[-1]*180/np.pi, -90], '--k')
plt.xlim([-180, 180])
plt.ylim([-90, 90])
...
# ax.add_patch(patch4)
# plt.hold(True)
# plt.plot(right_as_2[::10]*180/np.pi, dec_2[::10]*180/np.pi,'.',markersize=2,label='Camera 2 vector tip')
# plt.plot([right_as_2[0]*180/np.pi], [dec_2[0]*180/np.pi], marker='x', markersize=10, color="r",label='start')
# plt.plot([right_as_2[-1]*180/np.pi], [dec_2[-1]*180/np.pi], marker='x', markersize=10, color="g",label='end')
# ax.text(-97, -76, r'$1$',size=35)
# ax.text(-97, -5, r'$2$',size=35)
# ax.text(38, -5, r'$3$',size=35)
# ax.text(128, -5, r'$4$',size=35)
# # plt.plot([right_as_2[0]*180/np.pi, right_as_2[0]*180/np.pi], [dec_2[0]*180/np.pi, -90], '--k')
# # plt.plot([right_as_2[-1]*180/np.pi, right_as_2[-1]*180/np.pi], [dec_2[-1]*180/np.pi, -90], '--k')
# plt.xlim([-180, 180])
# plt.ylim([-90, 90])
# plt.xlabel(r'Right ascension [$^\circ$]',size=27)
# plt.ylabel(r'Declination [$^\circ$]',size=27)
# plt.legend(loc='upper right',prop={'size':22})
# plt.savefig('figures_regulation/map_2.pdf', bbox_inches='tight', dpi=300)
# plt.close()
...
plt.axhline(np.rad2deg(angle_thres_1), label=r'$\theta_{thres_{min1}}$', linestyle='--', color='c')
plt.axhline(np.rad2deg(angle_thres_1_max), label=r'$\theta_{thres_{max1}}}$', linestyle='--', color='m')
plt.ylim([0,180])
plt.legend(prop={'size':22})
plt.xlabel(r'$t$ [sec]', size=27)
...
plt.axhline(np.rad2deg(angle_thres_2), label=r'$\theta_{thres_{min2}}$', linestyle='--', color='c')
plt.axhline(np.rad2deg(angle_thres_2_max), label=r'$\theta_{thres_{max2}}}$', linestyle='--', color='m')
plt.ylim([0,180])
plt.legend(prop={'size':22})
plt.xlabel(r'$t$ [sec]', size=27)
...
plt.axhline(np.rad2deg(angle_thres_3), label=r'$\theta_{thres_{min3}}$', linestyle='--', color='c')
plt.axhline(np.rad2deg(angle_thres_3_max), label=r'$\theta_{thres_{max3}}}$', linestyle='--', color='m')
plt.ylim([0,180])
plt.legend(prop={'size':22})
plt.xlabel(r'$t$ [sec]', size=27)
